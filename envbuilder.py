...
class EnvironmentBuilder(object):
    ...
    def _clone_env(self, args):
        ...
        if not os.path.exists(self.path_to_workspace + os.sep + repo):
            self.handle_command('cd {0};git clone https://{1}:\'{2}\'@source.devsnc.com/{3}'.format(self.path_to_workspace,user,password,repo))
    ...
    def _switch_repo(self, args):
        ...
        ColorPrint.blue_highlight("Trying to switch the repository to [{0}]".format(track_name))
        if os.path.exists(self.path_to_workspace + os.sep + repo):
            ...
            if out == track_name:
                ColorPrint.warn("The current repository already switched to [{0}], skipping".format(track_name))
            else:
...
#                                                                                   """);
    ...
    parser = argparse.ArgumentParser(prog='envbuilder',
                                     description='ServiceNow build environemnt tool',
    ...
    if args.username and args.password and args.release and args.track:
        builder = EnvironmentBuilder(args.release)
        EnvironmentBuilder.print_list_avalable_versions()
